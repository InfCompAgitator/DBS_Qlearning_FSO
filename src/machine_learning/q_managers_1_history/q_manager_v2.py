...
"""Multi-agent rollout Q-Learning (Bertsekas)"""
...
class QManager:
    ...
    def __init__(self, uavs, uav_locs, energy_levels, reliability_levels, served_users_getter, testing_flag=QLearningParams.TESTING_FLAG,
        ...
        if not load_model:
            self.qs = []
            qs_shape = [self.uav_locs[0].size] * self.n_uavs + [self.reliability_levels.size] * self.n_uavs + [self.uav_locs[0].size] * self.n_uavs + [self.reliability_levels.size] * self.n_uavs + [self.uav_locs[0].size]  # discarding energy for now
            for uav_idx in range(self.n_uavs):
                self.qs.append(np.zeros(tuple(qs_shape), dtype=np.float32))
                qs_shape += [self.uav_locs[0].size]
        else:
            self.qs = []
            file_name = os.path.join(QLearningParams.CHECKPOINTS_FILE, self.__repr__() + '.npz')
            npzfile = np.load(file_name)
            for _arr in npzfile.files:
                self.qs.append(npzfile[_arr])
    ...
    def save_checkpoint(self, folder_name=QLearningParams.CHECKPOINTS_FILE):
        ...
        np.savez(file_name, *self.qs)

    def update_q_values(self, reward, cycle_idx):
        _, new_actions_idxs = self.select_actions(cycle_idx, testing=True)
        ...
        for idx in range(self.n_uavs):
            ...
            new_action_q = self.qs[idx][new_action_idx]
            old_q_value = self.qs[idx][last_action_idx]
            updated_q_value = old_q_value + learning_rate * (reward + discount_ratio * new_action_q - old_q_value)
            self.qs[idx][last_action_idx] = updated_q_value
    ...
    def select_actions(self, cycle_idx, testing=False):
        ...
        actions_last_idxs = []
        for idx in range(self.n_uavs):
            q_idx = old_locations_idxs + old_reliability_idxs + locations_idxs + reliability_idxs + actions_last_idxs
            q_values = self.qs[idx][tuple(q_idx)]
    ...
    @staticmethod
    def __repr__():
        return f'QManager_H1_V2_{QLearningParams.CHECKPOINT_ID}'
