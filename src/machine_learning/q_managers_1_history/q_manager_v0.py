...
"""Single-agent Q-Learning + double Q learning"""


class QManager:
    last_actions_idxs = None

    def __init__(self, uavs, uav_locs, energy_levels, reliability_levels, served_users_getter, testing_flag=QLearningParams.TESTING_FLAG,
        ...
        self.qs = [[], []]
        if not load_model:
            qs_shape = tuple([self.uav_locs[0].size] * self.n_uavs + [self.reliability_levels.size] * self.n_uavs + [self.uav_locs[0].size] * self.n_uavs + [self.reliability_levels.size] * self.n_uavs + [self.uav_locs[0].size ** self.n_uavs])  # discarding energy for now
            self.qs[0] = np.zeros(qs_shape, dtype=np.float32)
            self.qs[1]= np.zeros(qs_shape, dtype=np.float32)
        else:
            file_name = os.path.join(QLearningParams.CHECKPOINTS_FILE, self.__repr__() + '.npz')
            npzfile = np.load(file_name)
            self.qs[0] = npzfile[npzfile.files[0]]
            self.qs[1] = npzfile[npzfile.files[0]]
    ...
    def update_q_values(self, reward, cycle_idx):
        ...
        _, new_action_idx = self.select_actions(cycle_idx, testing=True, q_set=q_set_idx)
        ...
        new_action_q = self.qs[1 - q_set_idx][new_action_idx]
        old_q_value = self.qs[q_set_idx][last_action_idx]
        updated_q_value = old_q_value + learning_rate * (reward + discount_ratio * new_action_q - old_q_value)
        self.qs[q_set_idx][last_action_idx] = updated_q_value
    ...
    def select_actions(self, cycle_idx, testing=False, q_set=None):
        ...
        q_values = self.qs[0][q_idx] + self.qs[1][q_idx] if q_set is None else self.qs[q_set][q_idx]
    ...
    def get_location_states(self):
        ...
        for idx, _uav in enumerate(self.uavs):
            states[idx] = np.where([self.uav_locs[idx][i] == self.uavs[idx].coords for i in range(len(self.uav_locs[idx]))])[0][0]
        return states
    ...
    @staticmethod
    def __repr__():
        return f'QManager_H1_V01_{QLearningParams.CHECKPOINT_ID}'