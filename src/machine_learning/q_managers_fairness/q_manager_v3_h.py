...
class QManager:
    ...
    def __init__(self, uavs, uav_locs, energy_levels, reliability_levels, served_users_getter,
        ...
        if not load_model:
            ...
            for uav_idx in range(self.n_uavs):
                self.qs[0].append(np.zeros(tuple(qs_shape), dtype=np.float32))
                self.qs[1].append(np.zeros(tuple(qs_shape), dtype=np.float32))
    ...
    def get_reliability_levels(self):
        ...
        fairness_levels = np.floor(QLearningParams.FAIRNESS_LEVELS*per_uav_score).astype(int)
        return fairness_levels - 1, total_score * users_stats[1]

    def get_reward(self):
        return self.n_unserved * 100
    ...
    @staticmethod
    def __repr__():
        return f'QManager_H1_V3_{QLearningParams.CHECKPOINT_ID}'
