...
class QManager:
    ...
    def __init__(self, uavs, uav_locs, energy_levels, reliability_levels, served_users_getter,
        ...
        if not load_model:
            ...
            for uav_idx in range(self.n_uavs):
                self.qs[0].append(np.zeros(tuple(qs_shape), dtype=np.float16))
                self.qs[1].append(np.zeros(tuple(qs_shape), dtype=np.float16))
    ...
    def select_actions(self, cycle_idx, testing=False, q_set=None):
        locations_idxs = self.location_states
        reliability_idxs = self.reliability_states
        actions_idxs = []
        new_locations = []
        actions_last_idxs = []
        q_idx_init = locations_idxs + reliability_idxs.tolist()
        for idx in range(self.n_uavs):
            q_idx = q_idx_init + actions_last_idxs
    ...
    def get_reliability_levels(self):
        ...
        per_uav_score = per_uav_sum**2 / (users_stats[0][0] * per_uav_squared_sum) * (users_stats[0][0] -users_stats[0][1])/users_stats[0][0]
        ...
        return fairness_levels, total_score * users_stats[1]
